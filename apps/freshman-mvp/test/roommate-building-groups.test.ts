...
test('migration preserves building groups and roommate data through v7', () => {
  ...
  try {
    ...
    assert.equal(database.prepare('SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1').get()?.version, 7);
    assert.deepEqual(
