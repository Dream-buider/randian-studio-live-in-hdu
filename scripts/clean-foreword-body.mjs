...
await context.request.post('https://scnbcye3xdfz.feishu.cn/space/api/wiki/v2/tree/update_title/', {
  headers: {
...
await page.locator('.page-main-item.editor').click({ position: { x: 120, y: 245 } });
...
await page.evaluate(() => {
  window.webpackChunk.push([[Math.floor(Math.random() * 1e9)], {}, (req) => { window.__wreq = req; }]);
  const eventBus = window.__wreq(697656).Ay;
  const events = window.__wreq(472336).V;
  const coverTypes = window.__wreq(898831).K;
  eventBus.emit(events.SET_COVER_DATA, { type: coverTypes.NONE });
});
...
const data = await page.evaluate(() => {
  ...
  return {
    ...
    coverExists: !!document.querySelector('.doc-cover-wrapper'),
    duplicateCount: (body.match(/一个面向杭电学生的长期知识整理计划/g) || []).length,
    sectionCount: (body.match(/为什么会有 LIVE IN HDU|大学不会主动给你说明书|我们想留下什么/g) || []).length,
    bodyText: body.slice(0, 4500),
  };
});

console.log(JSON.stringify(data, null, 2));
await page.screenshot({ path: 'C:\\Users\\Star\\Desktop\\杭电飞书社区\\foreword-final-top.png', fullPage: false });
await context.close();
